...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db
    .from('orders')
    .select('id, customer_name, customer_phone, total, order_type, table_number, delivery_address, payment_method, special_notes, created_at, session_id, order_items(item_name, item_price, quantity)')
    .eq('staff_id', staffId)
